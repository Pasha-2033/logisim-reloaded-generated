...
public class PortParser {
        ...
        for (Component parentcomponent : WorkEnvironmentMain.ProjectComponents){
            for (Component component : parentcomponent.getintercomponentsandsircuts()){
                for (Port port : component.getPorts()){
                    int rx = Port.rotatedPort(result)[0] + result.belongsto.getComponentLocation()[0];
                    int ry = Port.rotatedPort(result)[1] + result.belongsto.getComponentLocation()[1];
                    int pdx = Math.abs(Port.rotatedPort(port)[0] + component.getComponentLocation()[0] - x);
                    int pdy = Math.abs(Port.rotatedPort(port)[1] + component.getComponentLocation()[1] - y);
    ...
    public static final Port getPort(int x, int y, int deltax, int deltay){
        ...
        for (Component parentcomponent : WorkEnvironmentMain.ProjectComponents){
            for (Component component : parentcomponent.getintercomponentsandsircuts()){
                for (Port port : component.getPorts()){
                    int portx = Port.rotatedPort(port)[0];
                    int porty = Port.rotatedPort(port)[1];
                    if (x + deltax < portx + component.getComponentLocation()[0] && x - deltay > portx + component.getComponentLocation()[0] && y + deltay < porty + component.getComponentLocation()[1] && y - deltay > porty + component.getComponentLocation()[1]) return port;
                }
            }
        }
        return new Port();
                ...
                if (component != othercomponent){
                    for (Port inport : component.getPorts()){
                        for (Port outport : othercomponent.getPorts()){
                            ...
                            int[] in = Port.rotatedPort(inport);
                            int[] out = Port.rotatedPort(outport);
                            if (in[0] + inport.belongsto.getComponentLocation()[0] == out[0] + outport.belongsto.getComponentLocation()[0] && in[1] + inport.belongsto.getComponentLocation()[1] == out[1] + outport.belongsto.getComponentLocation()[1]){
